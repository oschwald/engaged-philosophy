...
function the_bootstrap_comment_form_defaults( $defaults ) {
	return wp_parse_args( array(
		'comment_field'			=>	'<div class="comment-form-comment control-group"><label class="control-label" for="comment">' . _x( 'Comment', 'noun', 'the-bootstrap' ) . '</label><div class="controls"><textarea class="col-span-7" id="comment" name="comment" rows="8" aria-required="true"></textarea></div></div>',
...
function the_bootstrap_comment( $comment, $args, $depth ) {
	...
	if ( 'pingback' == $comment->comment_type OR 'trackback' == $comment->comment_type ) : ?>
		...
		<li id="li-comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
			<p class="row">
				<strong class="ping-label col-span-1"><?php _e( 'Pingback:', 'the-bootstrap' ); ?></strong>
				<span class="col-span-7"><?php comment_author_link(); edit_comment_link( __( 'Edit', 'the-bootstrap' ), '<span class="sep">&nbsp;</span><span class="edit-link label">', '</span>' ); ?></span>
			</p>
	...
	<?php else:
		...
		<li  id="li-comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
			<article id="comment-<?php comment_ID(); ?>" class="comment row">
				<div class="comment-author-avatar col-span-1<?php if ($offset) echo " offset{$offset}"; ?>">
