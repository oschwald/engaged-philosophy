document.addEventListener('DOMContentLoaded', function() {
	...
	wp.customize( 'the_bootstrap_theme_options[theme_layout]', function( value ) {
		value.bind( function( to ) {
			...
			if ( body ) {
				body.classList.remove( 'content-sidebar', 'sidebar-content' );
	...
	wp.customize( 'the_bootstrap_theme_options[navbar_site_name]', function( value ) {
		value.bind( function( to ) {
			const existingBrand = document.querySelector( 'span.navbar-brand' );
			if ( existingBrand ) {
			...
			if ( to ) {
				...
				if ( navbarCollapse ) {
					...
					navbarCollapse.parentNode.insertBefore( span, navbarCollapse );
				}
			}
		});
	});
	wp.customize( 'the_bootstrap_theme_options[navbar_searchform]', function( value ) {
		value.bind( function( to ) {
			...
			if ( to ) {
				const navbarCollapse = document.querySelector( '.navbar-collapse' );
				if ( navbarCollapse ) {
					navbarCollapse.innerHTML += the_bootstrap_customize.searchform;
				}
			}
		});
	});
		...
		value.bind( function( to ) {
			...
			if ( container ) {
				...
				if ( 'static' != to && navbar ) {
					...
					const margin = ( 'fixed-top' == to ) ? 'margin-top' : 'margin-bottom';
